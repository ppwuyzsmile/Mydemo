...
//   }
//
//   for (i = 0; ecc_sets[i].size; i++) {
//       #if 0
//          printf("Testing %d\n", ecc_sets[i].size);
//       #endif
//       if ((err = mp_read_radix(&modulus, (char *)ecc_sets[i].prime, 64)) != MP_OKAY)   { goto error; }
//       if ((err = mp_read_radix(&order, (char *)ecc_sets[i].order, 64)) != MP_OKAY)     { goto error; }
//
...
//       }
//
//       if ((err = mp_read_radix(&G->x, (char *)ecc_sets[i].Gx, 64)) != MP_OKAY)         { goto error; }
//       if ((err = mp_read_radix(&G->y, (char *)ecc_sets[i].Gy, 64)) != MP_OKAY)         { goto error; }
//       mp_set(&G->z, 1);
//
